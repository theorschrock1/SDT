...
joinDTs=function(x,y,all.x=F,all.y=F,all=F){

  setkeyv(x,names(x)%IN%names(y))
  setkeyv(y,names(x)%IN%names(y))
  DTmerge(x,y,all.x=all.x,all.y=all.x,all=all)
}
joinDTlist=function(x,all.x=F,all.y=F,all=F){
  reduce(x,joinDTs,all.x=all.x,all.y=all.x,all=all)
}
#' @export
get_measure_names=function(dt){
  dt%dtype_nms%("continuous"&"!date")
}
#' @export
get_identity_names=function(dt){
...
get_column_names=function(data){
  if(!is(data,'casted')){
    return(data.table(header1=get_measure_names(data)))
  }
  dtmp= data%get%get_casted_measures(data)
  out2= lapply(dtmp,attr,"measure")
  out2=lapply(dtmp,function(x){
    out<-rev(attr(x,'identity'))
    out$.measure_name=attr(x,"measure")
    get_uncasted_vars(data)
    out
  })
  names(out2)<-NULL
  out2=out2[!sapply(out2,is.null)]
  colnam=rev(names(out2[[1]]))
  out2=unique(as.data.table(stack_lists(out2)))
  setcolorder( out2,colnam)
  setnames(out2,rev(paste0('header',1:ncol(out2))))
  out2
